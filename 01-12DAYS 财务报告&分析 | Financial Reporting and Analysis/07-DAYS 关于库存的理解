...
IFRS and US GAAP都允许公司采用以下存货计价方法：
...
**根据Weighted Average Cost加权平均成本法，成本通过使用所有库存项目实际成本的加权平均组合分配到销售成本和期末库存：**
...
1. 衡量存货所采用的会计政策，包括所使用的成本公式（存货估价法）。
2. 存货的账面总金额和适合本企业的分类（如商品、原材料、生产用品、在制品和产成品）的账面金额。
3. 按公允价值减去销售成本后的存货账面金额。
4. 確認為期內開支的存貨金額（銷售成本）。
5. 確認為期內開支的任何存貨撇減金額。
6. 於期內確認為銷售成本減少的任何撇帳的撥回金額。
7. 導致撥回存貨撇帳的情況或事件；及
8. 作为负债担保的存货的账面金额；

US GAAP 的披露要求和IFRS相似，除了不能撤销减计。

### 6.2 存货比率

有三个比率经常用来衡量公司的经营状况：inventory turnover存货周转率、days of inventory on hand在手存货天数和gross margin毛利率。

存货周转率衡量的是公司在一年内出售（即周转）存货的次数。周转率越高，说明该存货在一年内销售的次数越多，存货资源的相对投资越少。在手存货天数可以计算为当期天数除以存货周转率。因此，存货周转率和库存天数成反比关系。但可能是存货周转率是用当年的平均存货计算的，而库存天数是根据期末存货金额计算的。一般来说，存货周转率和库存天数应以行业规范为基准，并进行跨年度比较。

存货比率高，库存天数少，可能表明该公司没有足够的存货，或该公司已将存货价值减记。

结合公司的销售增长情况，我们可以更加清楚地判断库存是否会短缺，如果公司销售增长变缓，同时存货周转率很高，那么预示着公司可能要库存不足了。

奢侈品销售公司的毛利率一般高于主食产品销售公司。但是，销售奢侈品的公司的存货周转率可能比销售主食产品的公司的存货周转率低得多。

### 6.3 财务分析示意

对于制造公司来说，这些分类可能包括生产用品、原材料、在制品和成品。对于零售商来说，这些分类可能包括重要的商品类别或具有类似属性的存货分组。这些披露可能提供有关公司未来销售和利润的信号。例如，原材料和(或)在制品库存的大幅增加(归因于单位数量的增加而不是单位成本的增加)可能意味着公司预计对其产品的需求将增加。这表明销售和利润预期会增加。

但是，如果在原材料和在制品存货下降的情况下，产成品存货大幅增加，则可能预示着公司产品的需求量会减少，从而降低未来的销售额和利润。这也可能预示着未来可能出现产成品存货的减记。

不管是哪种信号，分析师都应该彻底调查公司原材料、在制品和产成品存货发生任何重大变化的根本原因。

分析师还应该将公司的销售增长率与产成品存货的增长率进行比较，因为这也可以提供一个关于未来销售和利润的信号。例如，如果存货的增长速度大于销售的增长速度，这可能表明需求下降，未来的收益减少。公司可能不得不降低(减价)产品售价以减少存货余额，也可能因为陈旧而不得不减记存货价值，这两种情况都会对利润产生负面影响。除了可能出现的减价或减记外，手头存货过多或存货种类错误也会给公司带来负面的财务影响，因为这增加了与存货相关的费用，如保险、仓储成本和税收等。此外，这意味着公司可用于其他目的的现金和周转资金减少。

存货减值可能会对公司的活动性、盈利能力、流动性和偿付能力比率产生重大影响。分析师必须了解产品过时的行业趋势，并分析财务比率对潜在存货减值的敏感性。公司可以通过更好地将存货构成和增长与潜在的客户需求相匹配，将存货减值的影响降到最低。要获得有关公司存货及其未来销售的更多信息，有多种信息来源。分析师应考虑公司财务报告中的MD&A或类似部分、行业相关新闻和出版物以及行业经济数据。



## 总结

1. 存货是分析商品销售和生产企业的一个主要因素。这类公司定期通过存货交易产生销售和利润。确定这些公司利润的一个重要考虑因素是衡量存货销售时的销售成本。
2. 存货的总成本包括所有的购买成本、转换成本以及使存货达到目前位置和状态所发生的其他成本。产成品存货的储存成本和因浪费而产生的非正常成本，一般作为发生当期的费用处理。
3. 允许的存货计价方法隐含着对成本流量的不同假设。存货计价方法的选择决定了当期可供销售商品的成本如何在存货和销售成本之间进行分配。
4. 国际财务报告准则允许三种存货计价方法（成本公式）：先进先出法（FIFO）；加权平均成本法；具体识别法。具体识别法用于通常不能互换的物品的存货，以及为特定项目生产和分离的商品或服务的存货。美国会计准则允许上述三种方法加上后进先出法（LIFO）。在美国，后进先出法在税收和财务报告中被广泛使用，因为它可以节约所得税。
5. 存货法的选择会影响到财务报表和基于这些报表的任何财务比率。因此，分析人员在评价公司一段时间内的业绩或与行业数据或行业竞争对手比较时，必须仔细考虑存货计价方法的差异。
6. 公司必须对所有与实体具有类似性质和用途的存货使用相同的成本公式。
7. 当使用加权平均成本或后进先出存货计价方法时，存货会计制度(永续或定期)可能导致销售成本和期末存货的价值不同。
8. 根据US GAAP，使用后进先出法的公司必须在其财务说明中披露后进先出储备金的数额或如果使用先进先出法本应在库存中报告的数额。这些信息可用于调整已报告的后进先出法存货和销售成本余额，以便进行比较。
9. 当期末存货中的单位数量比年初存在的单位数量减少时，就会发生后进先出法清算。如果存货单位成本普遍逐年上升，就会产生与存货相关的毛利增加。
10. 国际财务报告准则和美国通用会计准则都要求存货成本计算的一致性。如果一个公司改变了会计政策，这种改变必须是合理的，并在财务报表中追溯应用。追溯重述的例外情况是，按照美国公认会计原则报告的公司改用后进先出法。
11. 根据《国际财务报告准则》，存货按成本和可变现净值两者中较低者计量。可变现净值是指在正常业务过程中的估计售价减去进行销售所需的估计成本。根 據 美 國 公 認 會 計 原 則，存 貨 按 成 本、市 值 或 可 變 現 淨 值（取 決 於 所 用 存 貨 方 法）較 低 者 計 量。市价是指当前的重置成本，以可变现净值为上限，以可变现净值为下限，减去正常利润率。根据《国际财务报告准则》，允许对以前的减记进行逆转，但根据美国公认会计原则则不允许。
12. 根据IFRS存货减记可能会发生逆转，但US GAAP不允许。
13. 存货分类（如原材料、在制品和产成品）中账面金额的变化可提供有关公司未来销售和利润的信号。有关存货管理和未来销售的相关信息，可以在年报或季报中的 "管理讨论与分析 "或类似项目、行业新闻和出版物以及行业经济数据中找到。
14. 存货周转率、存货天数比率、毛利率比率对评价公司的存货管理很有帮助。
15. 存货管理可能会对公司的活动性、盈利能力、流动性和偿债能力比率产生实质性影响。对分析师来说，了解行业趋势和管理层的意图至关重要。
16. 财务报表披露提供了有关计量存货所采用的会计政策、使用与存货有关的估计的主要不确定因素以及存货账面金额和成本的细节等信息。这些信息可以极大地帮助分析人员评估公司的存货管理情况。